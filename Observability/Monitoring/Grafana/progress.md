...
✅ Result: Grafana is now connected to Prometheus and receiving data successfully!
...
🔧 Troubleshooting: Ingress Nginx Dashboard
🔴 The Problem
When I tried to import the Ingress Nginx Dashboard (ID: 9614), it showed no data.
Root Cause Analysis:

❌ Ingress controller was not exposing metrics
❌ Prometheus was not scraping the Ingress controller


✅ The Solution
1️⃣ Enable Metrics on Ingress Controller
Added the following arguments to the Ingress controller deployment:
yamlargs:
  - --enable-metrics=true
  - --metrics-per-host=false
2️⃣ Expose Metrics Port on Ingress Service
...
✅ Ensure the service exposes its metrics (usually via a /metrics endpoint)
✅ Create a ServiceMonitor resource to tell Prometheus where to scrape
✅ Verify the metrics port is properly exposed on the service
